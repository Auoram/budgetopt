...
# Suppress the joblib/sklearn parallel config warning —
# it is a known sklearn internal issue, not a bug in our code.
...
def train(data_path: Path = DATA_PATH) -> dict:
    ...
    cpl_mae  = mean_absolute_error(y_cpl_test,  model_cpl.predict(X_test))
    conv_mae = mean_absolute_error(y_conv_test, model_conv.predict(X_test))
